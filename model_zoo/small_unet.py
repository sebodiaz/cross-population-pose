...
class Unet(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_downs):
            ...
            model += [
                Conv(
                    ...
                    bias=use_bias, padding=padding, padding_mode=pad_type,
                )
            ]
            self.res_source += [len(model) - 1]
            if Norm is not None:
        ...
        for i in range(num_downs):
            ...
            #if ndims == 4:
            #    model += [Upsample6D(scale_factor=2, mode=interp)]
            #else:
            model += [torch.nn.Upsample(scale_factor=2, mode=interp)]
...
class ConvBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.use_bias = bias
        assert ndims in [1, 2, 3], 'ndims in 1--4. found: %d' % ndims # changed to 4D
        #if ndims == 4:
        #    Conv = Conv4d
        #else:
        Conv = getattr(torch.nn, 'Conv%dd' % ndims)
